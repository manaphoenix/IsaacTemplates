...
function flawless:AddCallback(func) table.insert(callbacks, func) end
...
function flawless:init(mod)
    ...
    function mod:DamageTaken(ent, DamageAmount, DamageFlags, DamageSource,
                             DamageCountdownFrames, entType)
        if DamageAmount > 0 and not flawless.TookDamage and not room:IsClear() then
            flawless.TookDamage = true
        end
    end
    mod:AddCallback(ModCallbacks.MC_ENTITY_TAKE_DMG, mod.DamageTaken,

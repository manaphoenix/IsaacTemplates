...
local function GetRoomItem(defaultPool, AllowActives, MinQuality)
    ...
    if AllowActives == nil then AllowActives = true end
    ...
    if (not AllowActives or MinQuality > 0) then
        ...
        local active = (AllowActives == true) and true or itemConfig.Type ==
                           ItemType.ITEM_PASSIVE
        local quality = true
        if REPENTANCE then
            quality = MinQuality == 0 and true or itemConfig.Quality >=
                          MinQuality
        end
        while (not quality or not active) do
            ...
            active = (AllowActives == true) and true or itemConfig.Type ==
                         ItemType.ITEM_PASSIVE
            quality = MinQuality == 0 and true or itemConfig.Quality >=
                          MinQuality
        end
